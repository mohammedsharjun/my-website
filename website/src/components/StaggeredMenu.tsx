...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Hide prelayers by default
      if (preLayers.length) {
        gsap.set(preLayers, { opacity: 0, visibility: 'hidden' });
      }
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    // Show prelayers first
    if (layers.length) {
      tl.set(layers, { opacity: 1, visibility: 'visible' });
    }
  ...
  const playClose = useCallback(() => {
    ...
    closeTweenRef.current = gsap.to(all, {
      ...
      onComplete: () => {
        ...
        // Hide prelayers
        if (layers.length) {
          gsap.set(layers, { opacity: 0, visibility: 'hidden' });
        }
  ...
  return (
    <div className="sm-scope w-full h-full">
      ...
      >
        ...
        >
          <div className="sm-logo flex items-center text-black text-3xl font-bold select-none pointer-events-auto" aria-label="Logo">
            <img
              src={logoUrl || '/public/images/logo-bg.png'}
              alt="Logo"
              className="sm-logo-img block h-25 object-cover"
              draggable={false}
              width={110}
              height={50}
            /> Benryx
...
.sm-scope .staggered-menu-header { position: absolute; top: 0; left: 0; width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 0.5em; background: transparent; pointer-events: none; z-index: 20; }
